...
import { Shield, Swords, Flame, ShoppingCart, Coffee, Skull, HelpCircle, Package, ArrowUp } from 'lucide-react';
import { MAP_NODES, ARTIFACTS } from '../data/gameData';

export default function GameMap({ currentTier, visitedNodes, onSelectNode, character, playerArtifacts }) {
    ...
    <div className="flex flex-col items-center justify-between min-h-screen bg-slate-900/90 text-white p-4 relative overflow-y-auto">
      <div className="w-full max-w-4xl bg-slate-950/80 p-3 rounded-xl border border-slate-800 flex justify-between items-center mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-bold text-sm text-slate-200">{character.name}</div>
        ...
        <div className="flex-1 px-4 flex justify-center gap-1.5 border-x border-slate-800/60 overflow-x-auto max-w-sm">
          {playerArtifacts && playerArtifacts.length > 0 ? (
            playerArtifacts.map(artId => {
              const artifact = ARTIFACTS[artId];
              return (
                <div key={artId} className="group relative w-7 h-7 rounded bg-slate-800 border border-slate-700 flex items-center justify-center text-amber-400 cursor-help transform hover:scale-110 transition-transform">
                  <Package size={16} />
        ...
        <div className="text-right">
          <div className="text-xs text-slate-400 font-mono">Aktuelle Ebene</div>
          <div className="text-sm font-bold text-amber-500">Tier {currentTier + 1} / 7</div>
        </div>
      </div>

      <div className="flex-1 w-full max-w-3xl flex flex-col-reverse justify-around items-center my-4 relative bg-slate-950/40 rounded-2xl p-6 border border-slate-800/50 backdrop-blur-sm min-h-[600px] overflow-hidden">
        {tiers.map((tierIndex) => {
          ...
          return (
            <div key={tierIndex} className="w-full flex justify-center gap-6 relative py-3">
              <div className="absolute -top-1 left-0 right-0 border-t border-dashed border-slate-800/60 text-[10px] text-slate-600 px-2 flex justify-between">
                <span>{tierIndex === 6 ? 'BOSS' : tierIndex === 0 ? 'START' : `Ebene ${tierIndex}`}</span>
              </div>
      ...
      <div className="text-center text-xs text-slate-500 font-mono mt-2 max-w-md">
        Es muss ein leuchtender Knotenpunkt ausgewählt werden, um fortzufahren. Besiege den Boss auf Stufe 7, um den Run abzuschließen!
      </div>
    </div>
  );
}
